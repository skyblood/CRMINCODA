...
console.log(`[CRM] 🔄 Syncing ${JSON.stringify(pendingQueue.length)} pending operations to MongoDB...`);
 ...
 * Uses ?page=1&limit=100 — if the server returns more pages they are fetched in
 * parallel and merged. Completely transparent to callers: always returns a plain array.
 */
const fetchCollection = async (collectionName: string, limit = 100): Promise<any[]> => {
...
const COLLECTIONS = {
    ...
    PIPELINES: 'pipelines',
    // Note: automationRules → /api/automations, emailTemplates → /api/email-templates,
    // proposalTemplates → /api/proposal-templates (kebab routes — not fetched via localStore,
    // only handled by server-side GET /api/backup and POST /api/backup/restore)
};
...
/**
 * 💾 BACKUP
 * When online: fetches ALL collections directly from MongoDB via GET /api/backup.
 * When offline: falls back to the client-side localStore cache (may be incomplete).
 */
export const backupLocalData = async (): Promise<string> => {
    if (await checkApiAvailability()) {
        try {
            const data = await apiFetch('/backup');
            return JSON.stringify(data, null, 2);
        } catch (e) {
            console.warn('[CRM] Backup API failed, falling back to localStore', e);
        }
    }
...
export const restoreLocalData = (jsonString: string): boolean => {
    try {
        ...
        (async () => {
            if (await checkApiAvailability()) {
                try {
                    await apiFetch('/backup/restore', { method: 'POST', body: JSON.stringify(dataToRestore) });
...
export const addDocument = async (collectionName: string, data: any) => {
    ...
    if (await checkApiAvailability()) {
        try {
            await apiFetch(`/${collectionName}`, { method: 'POST', body: JSON.stringify(newItem) });
